...
describe("Pruebas de colisiones", function(){
	...
	it("misil destruye nave", function(){
		...
		gameBoard.step(0.1);
		expect(gameBoard.removed.length).toEqual(2);//al chocar el missile se añade a objetos a 
		expect(gameBoard.objects.length).toEqual(1);//1 objeto queda(la explosion que se genera), el misil al chocar se borra junto al enemigo que también es eliminado
		expect(enemy.health).toEqual(-5);//le resta a 20-10(daño del misil) = 10 (vida del enemigo restante)*
	});
	...
	it("misil no destruye nave y desaparece", function(){
		...
		gameBoard.step(0.1);
		expect(gameBoard.removed.length).toEqual(1);//al chocar el missile se añade a objetos a borrar
		expect(gameBoard.objects.length).toEqual(1);//1 objeto queda(enemy), el misil al chocar se borra
		expect(enemy.health).toEqual(50);//le resta a 60-10(daño del misil) = 50 (vida del enemigo restante)
	});
	...
	it("bola de fuego destruye nave y no desaparece", function(){
		...
		gameBoard.step(0.1);
		expect(gameBoard.removed.length).toEqual(1);//al chocar la bola elimina al enemigo y se añade a objetos a borrar
