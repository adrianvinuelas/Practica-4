...
var enemies = {
    ...
    mienemigo: { x: 50, y: -50, sprite: 'enemy_bee', health: 20, 
		A:10 ,B: 50, C: 1, E: 30 , F: 100, G: 1, H: Math.PI/2}
};
...
var level2 = [
  //  Comienzo, Fin,   Frecuencia,  Tipo,       Override
    [ 0,        4000,  500,         'step'                 ],
    [ 0,        4000,  500,         'step',     {x:300}    ],
    [ 6000,     13000, 800,         'ltr'                  ],
    [ 8000,     15000, 700,         'mienemigo',           ],
    [ 10000,    16000, 400,         'circle'               ],
    [ 17800,    20000, 500,         'straight', { x: 50  } ],
    [ 18200,    20000, 500,         'straight', { x: 90  } ],
    [ 18200,    20000, 500,         'straight', { x: 10  } ],
    [ 21900,    25000, 400,         'wiggle',   { x: 200 } ],
    [ 22000,    25000, 400,         'wiggle',   { x: 150 } ],
    [ 22000,    25000, 400,         'wiggle',   { x: 100 } ]
];

var niveles = [ level1 , level2];
var numnivel = 0;
var playGame = function() {
    
    var nivel = niveles[numnivel];
    ...
    // ganado el juego
    board.add(new Level(nivel, newLevel));
    Game.setBoard(3,board);
};

var newLevel = function(){
	  numnivel += 1;
	  var nombrenivel = numnivel+1
	  if(numnivel == niveles.length){
		winGame();
	  }else{
	  	Game.setBoard(3,new TitleScreen("Level"+ nombrenivel +",Ready?", 
                                    "Press fire to start playing",
                                    playGame));
	  }
};
...
var PlayerShip = function() { 
	...
	if(Game.keys['bolafuegoizq'] && !this.presionado && this.reload < 0){
		this.reload = this.reloadTime;
		this.presionado = true; // como he disparado presionado = true
		this.board.add(new FireBall(this.x,this.y+this.h/2, -1.5));
	}

	if(Game.keys['bolafuegoder'] && !this.presionado && this.reload < 0){
		this.reload = this.reloadTime;
		this.presionado = true; // como he disparado presionado = true
		this.board.add(new FireBall(this.x+this.w,this.y+this.h/2,1.5));
	}
    };
};
...
PlayerShip.prototype.hit = function(damage) {
	...
	this.board.add(new Explosion(this.x + this.w/2, 
                                     this.y + this.h/2));
	setTimeout(function(){loseGame()},1500);
...
var FireBall = function(x,y,direccion) {
    this.setup('explosion',{ vy: -700 , vx: 100 * direccion, damage: Infinity});
    this.x = x - this.w/2; 
    this.y = y - this.h;     
};
